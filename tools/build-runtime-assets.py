        ...
        for path in sorted(paths, key=lambda item: item.name):
            with Image.open(path) as image:
                ...
                if match:
                    ...
                    old = f"/common/sd/{path.name}"
                    replacements[old] = f"/common/sd/{representative.name}"
...
def repack_runtime(requested_colors: str = "auto", install_stable: bool = False) -> int:
    ...
    try:
        ...
        report = {
            ...
            "estimates": estimates,
            "folders": {folder: {"files": len(list((COMMON / folder).rglob("*.png"))), "bytes": runtime_tree_size(COMMON / folder)} for folder in ("bg", "ev", "sd", "overlay")},
        }
        GENERATED.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    # Backgrounds in the extracted archive are JPEGs with more than 64 colors.
    # Use the existing runtime PNGs as the canonical input so palette conversion
    # remains lossless and never introduces another JPEG decode/re-encode pass.
    runtime_bg_indexes = by_stem(index_files([COMMON / "bg"])['bg'])
    runtime_ev_indexes = by_stem(index_files([COMMON / "ev"])['ev'])
