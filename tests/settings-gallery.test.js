...
test('choice page has an explicit full-screen box for every device shape', () => {
  const root = path.resolve(__dirname, '..')
  const source = fs.readFileSync(path.join(root, 'src', 'pages', 'detail', 'detail.ux'), 'utf8')
  const choice = source.match(/\.choice-page \{([^}]+)\}/)?.[1] || ''
  assert.match(choice, /top:\s*0px/)
  assert.match(choice, /left:\s*0px/)
  assert.match(choice, /width:\s*(?:212px|100%)/)
  assert.match(choice, /height:\s*(?:520px|100%)/)
    assert.doesNotMatch(choice, /z-index:\s*\d+/)
})

test('reader save and load routes replace the reader instead of retaining image layers', () => {
  const detail = fs.readFileSync(path.join(root, 'src', 'pages', 'detail', 'detail.ux'), 'utf8')
  const saves = fs.readFileSync(path.join(root, 'src', 'pages', 'saves', 'saves.ux'), 'utf8')
  assert.match(detail, /router\.replace\(\{ uri: 'pages\/saves', params: \{ mode: 'save', resume: '1' \} \}\)/)
  assert.match(detail, /router\.replace\(\{ uri: 'pages\/saves', params: \{ mode: 'load', resume: '1' \} \}\)/)
  assert.match(saves, /router\.replace\(\{ uri: 'pages\/detail', params: \{ load: String\(slot\) \} \}\)/)
  assert.match(saves, /router\.replace\(\{ uri: 'pages\/detail', params: \{ auto: '1' \} \}\)/)
})

test('background changes mark progress without synchronous storage writes', () => {
  const detail = fs.readFileSync(path.join(root, 'src', 'pages', 'detail', 'detail.ux'), 'utf8')
  const background = detail.match(/case SCN_TYPE\.BACKGROUND:([\s\S]*?)case SCN_TYPE\.DIALOGUE:/)?.[1] || ''
  assert.match(background, /this\.markAutoSaveDirty\(\)/)
  assert.doesNotMatch(background, /this\.persistAutoSave\(true\)/)
  assert.match(detail, /signature === this\.lastAutoSaveSignature/)
})

test('Band 10 sources avoid unsupported z-index styles', () => {
  const pages = fs.readdirSync(path.join(root, 'src', 'pages'))
  for (const page of pages) {
    const file = path.join(root, 'src', 'pages', page, `${page}.ux`)
    if (fs.existsSync(file)) assert.doesNotMatch(fs.readFileSync(file, 'utf8'), /z-index:\s*\d+/)
  }
})

test('direct SD references resolve to real runtime assets', () => {
  const root = path.resolve(__dirname, '..')
  const manifest = JSON.parse(fs.readFileSync(path.join(root, 'tools', 'generated', 'resource-manifest.json'), 'utf8'))
  const sd = Object.entries(manifest.paths.sd || {})
  assert.ok(sd.length > 0)
  for (const [name, target] of sd) {
    assert.match(name, /^sd\d+/)
    assert.ok(target && !target.includes('transparent.png'))
    assert.ok(fs.existsSync(path.join(root, target.slice(1))) || fs.existsSync(path.join(root, 'build', target.slice(1))))
  }
})
